...
def record(model, data_loader, predictor, output_folder, device, timer=None, logger=None):
    model.roi_heads.relation.predictor.bias = {}
    for i in range(1, model.roi_heads.relation.predictor.num_tree):
        model.roi_heads.relation.predictor.bias[i] = np.zeros((model.roi_heads.relation.predictor.num_children[i], model.roi_heads.relation.predictor.num_children[i]))
    model.eval()
    ...
    for itr, batch in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            ...
            if timer:
                ...
                timer.toc()
    np.save(os.path.join(output_folder, "bias.npy"), model.roi_heads.relation.predictor.bias)
    torch.cuda.empty_cache()
    return 
...
def main():
    ...
    assert cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR=="CausalPSKTPredictor" and cfg.MODEL.ROI_RELATION_HEAD.CAUSALPSKT.KNOWLEDGE_TRANSFER==True and cfg.MODEL.ROI_RELATION_HEAD.CAUSALPSKT.EFFECT_TYPE=="TDE"
    ...
    model.roi_heads.relation.bias_record = True
    model.roi_heads.relation.predictor.bias_record = True
    ...
    if cfg.OUTPUT_DIR:
        output_folder = os.path.join(cfg.OUTPUT_DIR, "bias_record")
        mkdir(output_folder)
