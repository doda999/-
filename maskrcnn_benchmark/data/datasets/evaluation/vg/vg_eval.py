...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    local_container['rel_labels'] = prediction.get_field('pred_rel_labels').detach().cpu().numpy()          # (#pred_rels, )
    local_container["rel_max_scores"] = prediction.get_field('pred_rel_max_scores').detach().cpu().numpy()  # (#pred_rels, )
